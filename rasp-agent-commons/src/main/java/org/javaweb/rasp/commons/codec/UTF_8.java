...
import static org.javaweb.rasp.commons.codec.Characters.*;
...
public final class UTF_8 extends Unicode {
	...
	private static class Decoder extends CharsetDecoder {
		...
		private CoderResult decodeArrayLoop(ByteBuffer src,
			...
			int n = decodeASCII(sa, sp, da, dp, Math.min(sl - sp, dl - dp));
			sp += n;
			dp += n;
			...
			while (sp < sl) {
				...
				if (b1 >= 0) {
					...
					char c = (char) ((b1 << 12) ^ (b2 << 6) ^ (b3 ^ (((byte) 0xE0 << 12) ^
					...
					int uc = (
							(b1 << 18) ^ (b2 << 12) ^ (b3 << 6) ^ (b4 ^ (((byte) 0xF0 << 18) ^
									((byte) 0x80 << 12) ^ ((byte) 0x80 << 6) ^ ((byte) 0x80)))
					);
	...
	private static final class Encoder extends CharsetEncoder {
		...
		private CoderResult encodeArrayLoop(CharBuffer src, ByteBuffer dst) {
			...
			int n = implEncodeAsciiArray(sa, sp, da, dp, Math.min(sl - sp, dl - dp));
			sp += n;
			dp += n;
