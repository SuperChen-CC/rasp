package org.javaweb.rasp.commons.html.parser;
...
import org.javaweb.rasp.commons.html.nodes.Document;
import org.javaweb.rasp.commons.html.nodes.DocumentType;
import org.javaweb.rasp.commons.html.nodes.Element;

import java.util.ArrayList;

import static org.javaweb.rasp.commons.html.internal.StringUtil.inSorted;
import static org.javaweb.rasp.commons.html.parser.HtmlTreeBuilderState.Constants.*;
...
enum HtmlTreeBuilderState {
    ...
    InBody {
        ...
        private boolean inBodyStartTag(Token t, HtmlTreeBuilder tb) {
            ...
            if ("a".equals(name)) {
            ...
            } else if ("span".equals(name)) {// same as final else, but short circuits lots of checks
                tb.reconstructFormattingElements();
                tb.insert(startTag);
                ...
                for (int i = stack.size() - 1; i > 0; i--) {
                    ...
                    if (tb.isSpecial(el) && !inSorted(el.normalName(), InBodyStartLiBreakers))
                        break;
                }
                ...
                if (tb.getFromStack("svg") == null)
                    ...
                    tb.insert(startTag);
            } else if ("isindex".equals(name)) {// how much do we care about the early 90s?
                tb.error(this);
                ...
                if (startTag.hasAttributes()) {
                    for (Attribute attr : startTag.attributes) {
                        if (!inSorted(attr.getKey(), InBodyStartInputAttribs))
                ...
                tb.framesetOk(false);
                handleRawtext(startTag, tb);
            } else if ("noembed".equals(name)) {// also handle noscript if script enabled
                handleRawtext(startTag, tb);
            } else if ("select".equals(name)) {
                tb.reconstructFormattingElements();
                tb.insert(startTag);
                tb.framesetOk(false);
                if (startTag.selfClosing) return true; // don't change states if not added to the stack
            ...
            } else if ("h1".equals(name) || "h2".equals(name) || "h3".equals(name) || "h4".equals(name) || "h5".equals(name) || "h6".equals(name)) {
                if (tb.inButtonScope("p")) {
                    tb.processEndTag("p");
                }
                if (inSorted(tb.currentElement().normalName(), Headings)) {
                    tb.error(this);
                    tb.pop();
                }
                tb.insert(startTag);
                // static final String[] InBodyStartPreListing = new String[]{"listing", "pre"};
            } else if ("pre".equals(name) || "listing".equals(name)) {
                if (tb.inButtonScope("p")) {
                    tb.processEndTag("p");
                }
                tb.insert(startTag);
                ...
                for (int i = bottom; i >= upper; i--) {
                    el = stack.get(i);
                    if (inSorted(el.normalName(), DdDt)) {
            ...
            } else if ("optgroup".equals(name) || "option".equals(name)) {
            ...
            } else if ("area".equals(name) || "br".equals(name) || "embed".equals(name) || "img".equals(name) || "keygen".equals(name) || "wbr".equals(name)) {
                tb.reconstructFormattingElements();
                tb.insertEmpty(startTag);
                tb.framesetOk(false);
                // Formatters:
            } else if ("b".equals(name) || "big".equals(name) || "code".equals(name) || "em".equals(name) || "font".equals(name) || "i".equals(name) || "s".equals(name) || "small".equals(name) || "strike".equals(name) || "strong".equals(name) || "tt".equals(name) || "u".equals(name)) {
                tb.reconstructFormattingElements();
                el = tb.insert(startTag);
                tb.pushActiveFormattingElements(el);
                ...
                if (!Tag.isKnownTag(name)) { // no special rules for custom tags
                    tb.insert(startTag);
                } else if (inSorted(name, InBodyStartPClosers)) {
                    if (tb.inButtonScope("p")) {
                        tb.processEndTag("p");
                    }
                    tb.insert(startTag);
                    ...
                    return tb.process(t, InHead);
                } else if (inSorted(name, InBodyStartApplets)) {
                    tb.reconstructFormattingElements();
                    tb.insert(startTag);
                    tb.insertMarkerToFormattingElements();
                    tb.framesetOk(false);
                } else if (inSorted(name, InBodyStartMedia)) {
                    tb.insertEmpty(startTag);
                } else if (inSorted(name, InBodyStartDrop)) {
                    tb.error(this);
                    return false;
                } else {
                    tb.reconstructFormattingElements();
        ...
        private boolean inBodyEndTag(Token t, HtmlTreeBuilder tb) {
            ...
            if ("template".equals(name)) {
                tb.process(t, InHead);
            } else if ("sarcasm".equals(name) || "span".equals(name)) {// same as final fall through, but saves short circuit
                ...
                if (inSorted(name, InBodyEndAdoptionFormatters)) {
                    return inBodyEndTagAdoption(t, tb);
                } else if (inSorted(name, InBodyEndClosers)) {
                    if (!tb.inScope(name)) {
                        // nothing to close
                        tb.error(this);
                        return false;
    ...
    InColumnGroup {
        boolean process(Token t, HtmlTreeBuilder tb) {
            ...
            switch (t.type) {
                ...
                case StartTag:
                    Token.StartTag startTag = t.asStartTag();
                    if ("html".equals(startTag.normalName())) {
                        return tb.process(t, InBody);
                    } else if ("col".equals(startTag.normalName())) {
                        tb.insertEmpty(startTag);
                    } else if ("template".equals(startTag.normalName())) {
                        tb.process(t, InHead);
                    } else {
                        return anythingElse(t, tb);
                    }
    ...
    InFrameset {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (isWhitespace(t)) {
                ...
                Token.StartTag start = t.asStartTag();
                if ("html".equals(start.normalName())) {
                    return tb.process(start, InBody);
                } else if ("frameset".equals(start.normalName())) {
                    tb.insert(start);
                } else if ("frame".equals(start.normalName())) {
                    tb.insertEmpty(start);
                } else if ("noframes".equals(start.normalName())) {
