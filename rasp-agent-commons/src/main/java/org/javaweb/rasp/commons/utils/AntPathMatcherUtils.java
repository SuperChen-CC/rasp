...
public class AntPathMatcherUtils {
	...
	/**
	 * Default path separator: Linux:"/"，Windows:"\".
	 */
	public static final String DEFAULT_PATH_SEPARATOR = File.separator;
	 ...
	 * into this matcher's {@link #pathMatch} method. A value of {@code true}
	...
	public static boolean pathMatch(String pattern, String path) {
		if (isNotEmpty(path) && isNotEmpty(path)) {
			return new AntPathMatcherUtils().doMatch(pattern, path, true, null);
		}

		return false;
	}

	public static boolean pathMatchStart(String pattern, String path) {
		if (isNotEmpty(path) && isNotEmpty(path)) {
			return new AntPathMatcherUtils().doMatch(pattern, path, false, null);
		}
	 ...
	 * <p>Assumes that {@link #pathMatch} returns {@code true} for '{@code pattern}' and '{@code path}', but
	...
	public String combine(String pattern1, String pattern2) {
		...
		if (!pattern1.equals(pattern2) && !pattern1ContainsUriVar && pathMatch(pattern1, pattern2)) {
