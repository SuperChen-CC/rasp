 ...
 * log files depending on {@link RASPRollingPolicy} and {@link TriggeringPolicy}.
 * <p/>
 * <p/>
 ...
 */
public class RASPRollingFileAppender<E> extends RASPFileAppender<E> {

	File currentlyActiveFile;

	TriggeringPolicy<E> triggeringPolicy;

	RASPRollingPolicy rollingPolicy;

	private static String RFA_NO_TP_URL = CODES_URL + "#rfa_no_tp";

	private static String RFA_NO_RP_URL = CODES_URL + "#rfa_no_rp";

	private static String COLLISION_URL = CODES_URL + "#rfa_collision";

	private static String RFA_LATE_FILE_URL = CODES_URL + "#rfa_file_after";
	...
	private boolean checkForFileAndPatternCollisions() {
		if (triggeringPolicy instanceof RASPRollingPolicyBase) {
			final RASPRollingPolicyBase base            = (RASPRollingPolicyBase) triggeringPolicy;
			...
			if (!file.exists()) {
				try {
					if (!file.createNewFile()) {
						throw new RuntimeException("Can not create log file : " + file.getAbsolutePath());
					}
	...
	public RASPRollingPolicy getRollingPolicy() {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void setRollingPolicy(RASPRollingPolicy policy) {
	...
	public void setTriggeringPolicy(TriggeringPolicy<E> policy) {
		...
		if (policy instanceof RASPRollingPolicy) {
			rollingPolicy = (RASPRollingPolicy) policy;
		}
	}
