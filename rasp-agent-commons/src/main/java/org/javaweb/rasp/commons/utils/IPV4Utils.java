...
public class IPV4Utils {
	 ...
	 *
	 * @return 网卡信息
	 * @throws SocketException 获取网卡信息异常
	 */
	public static Map<String, Object> getNetworkInterface() throws SocketException {
		...
		try {
			...
			networkMap.put("hostname", hostName);
		} catch (UnknownHostException e) {
			networkMap.put("hostname", "Unknown");
		}
			...
			if (macBytes != null) {
				...
				for (InterfaceAddress interfaceAddress : interfaceAddresses) {
					...
					if (!ipAddress.isLoopbackAddress()) {
						interfaceMap.put("name", networkInterface.getName());
						interfaceMap.put("netmask", interfaceAddress.getNetworkPrefixLength());
						interfaceMap.put("ip", ipAddress.getHostAddress());
						interfaceMap.put("mac", macBuilder.toString());
						interfaceMap.put("broadcast", broadcast != null ? broadcast.getHostAddress() : "");
						interfaceMap.put("local", ipAddress.isSiteLocalAddress());
						interfaceMap.put("ipv4", ipAddress instanceof Inet4Address);

						interfaceList.add(interfaceMap);
					}
				}

				networkMap.put("interfaces", interfaceList);
			}
		}
